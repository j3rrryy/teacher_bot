...
from src.external_services import get_material
...
async def start(message: Message):
    try:
        ...
        await message.answer(
            f"Привет, {(await message.get_user()).first_name}👋\n\n"
            + LEXICON_RU["start"],
...
async def profile(message: Message):
    try:
        ...
        position = (
            LEXICON_RU["position"]
            + str(await get_top(message.from_id, message.__dict__["postgres"]))
            + "\n\n"
        )
        registered = LEXICON_RU["registered"] + profile["registered"].strftime(
            "%d.%m.%Y"
        )
...
async def heads_or_tails_res(message: Message):
    try:
        if heads_or_tails_game(message.text)[0]:
            new_rating = (
                await update_rating(message.from_id, 10, message.__dict__["postgres"])
            )["rating"]
            await message.answer(
                LEXICON_RU["win"] + str(new_rating),
